...
function Sidebar({ collapsed, onToggle, activeId, onNavigate }) {
  return (
    <aside className={`
...
<PageHeader
  ...
  actions={<>
    <Button variant="outline" icon={<Upload size={15}/>}>Import Data</Button>
...
const columns = [
  ...
  { key: "assignee", label: "Assignee", render: (v) => <Avatar name={v} size="sm" /> },
...
6. **Settings**: Always uses a two-panel layout — settings sidebar (left) + content (right).
7. **Profile**: A settings sidebar item, but also surfaceable as a topbar dropdown destination.
8. **Table rows**: Hover reveals action buttons (`opacity-0 group-hover:opacity-100`).
9. **Modals**: Always `backdrop-blur-sm`, `rounded-2xl`, close on Escape and backdrop click.
...
14. **Responsive hints**: On narrow screens, hide labels with `hidden md:block`; collapse sidebar by default on mobile.
15. **Skeleton shapes must match**: Every skeleton component mirrors the exact layout of its real counterpart — same padding, same rounded corners, same grid structure.
16. **Shimmer not pulse**: Use the gradient shimmer animation (see `Skeleton` base) not Tailwind `animate-pulse` — it looks more polished and on-brand.
17. **Loading hierarchy**: Initial load → `SplashLoader`. Page nav → `TopProgressBar`. First data fetch → skeleton. Refetch → `LoadingOverlay`. Button action → `LoadingButton`. Never use a full-screen spinner for a card refetch.
